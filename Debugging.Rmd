...
- Updating fits from trees, but **not** updating $u$ or $Omega$
  - Omega fixed at initialization
...
xx <- 1
  ...
  p <- data.frame(y = c(rowMeans(ydensobs), den$y),
    ...
    geom_ribbon(data = data.frame(dens95 = apply(ydensobs, 1, quantile, 0.95),
                ...
                fill = "firebrick") +
    #geom_histogram(data = data.frame(yobs = c(colMeans(mbf$yhat_post[bseq,x0==xx]),
    #                                          y[x0==xx]),
    #                                 xobs = c(x0[x0==xx],
    #                                          x0[x0==xx]),
    #                                 type = c(rep("Predicted Y", sum(x0==xx)),
    #                                          rep("True Y", sum(x0==xx)))),
    #               aes(x = yobs, y = after_stat(density), fill = factor(type)),
