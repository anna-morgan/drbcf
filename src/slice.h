...
class ld_dart_alpha: public logdensity {
  ...
  ld_dart_alpha(std::vector<double> counts_, std::vector<double> probs_) { 
    ld_dart_alpha(counts_, probs_, dart_a = 0.5, dart_b = 1, dart_rho = counts_.size());
  }
  ld_dart_alpha(std::vector<double> counts_, std::vector<double> probs_,
  ...
  double val(double y) { 
    ...
    for(size_t i=0; i<counts.size(); ++i) {
      double totct = counts[i] + y/dart_rho;
      csum += totct;
